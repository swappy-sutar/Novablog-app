...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      setPlaceholder(currentPhrase.substring(0, charIdx + (isDeleting ? -1 : 1)));
    }, typingSpeed);
    ...
    <nav className="fixed top-0 w-full z-50 glass-panel !rounded-none border-t-0 border-x-0 bg-bg-base/70">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium">
          <Link to="/" className="text-brand-cyan relative after:absolute after:-bottom-2 after:left-0 after:w-full after:h-0.5 after:bg-brand-cyan after:rounded-full">Feed</Link>
          <Link to="#" className="text-gray-400 hover:text-white transition-colors">Explore</Link>
          <Link to="#" className="text-gray-400 hover:text-white transition-colors">Docs</Link>
          <Link to="#" className="text-gray-400 hover:text-white transition-colors">Pricing</Link>
        </div>

        {/* Actions */}
        <div className="flex items-center gap-4">
          <div className="hidden lg:flex relative items-center group">
            <svg className="w-4 h-4 absolute left-3 text-gray-400 group-focus-within:text-brand-cyan transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <AnimatePresence mode="wait">
              {user ? (
                ...
                >
                  ...
                  <Link
                    to="/profile/settings"
                    className="text-sm font-medium text-gray-400 hover:text-white transition-colors hidden lg:inline"
                  >
                    Settings
                  </Link>
                  <button
                    ...
                    className="p-2 rounded-full text-gray-400 hover:text-white hover:bg-border-subtle transition-colors"
                    title="Log out"
                    aria-label="Log out"
                  >
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
