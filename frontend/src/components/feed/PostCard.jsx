...
const PostCard = ({ post, variant = "grid" }) => {
  ...
  return (
    <GlassCard className="relative overflow-hidden group h-full flex flex-col justify-between border border-border-subtle bg-bg-card p-5 hover:border-brand-purple/40 hover:-translate-y-1 hover:shadow-[0_0_20px_rgba(139,92,246,0.15)] transition-all duration-300">
      {/* Card background thumbnail layer - full bleed cover image */}
      <div className="absolute inset-0 z-0 select-none pointer-events-none">
        ...
        />
        <div className="absolute inset-0 transition-colors duration-500 postcard-overlay" />
      </div>
      ...
      <div className="relative z-10 flex flex-col justify-between h-full w-full">
        <Link to={`/post/${id}`} className="block space-y-3 group/link cursor-pointer">
          {/* Category */}
          <span className="text-[10px] font-bold text-brand-cyan tracking-wider uppercase block postcard-text-cyan">
            {category}
          </span>

          {/* Title */}
          <h3 className={`font-bold tracking-tight leading-snug group-hover/link:text-brand-cyan transition-colors postcard-text-white ${
        ...
        {/* Footer info */}
        <div className="flex items-center justify-between text-xs mt-5 pt-3 border-t postcard-border-subtle">
          <Link 
          ...
          >
            ...
            <span className="font-medium group-hover/author:text-brand-purple transition-colors truncate max-w-[120px] postcard-text-white">
              {author}
            </span>
          </Link>

          {!isCompact && (
            <span className="flex items-center gap-1 text-[11px] shrink-0 postcard-text-gray">
