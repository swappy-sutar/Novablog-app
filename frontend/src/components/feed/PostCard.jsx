...
const PostCard = ({ post, variant = "grid" }) => {
  const { id, title, category, author, avatarUrl, readTime, authorUsername } = post;
  const isCompact = variant === "compact";

  return (
    <GlassCard className="h-full flex flex-col justify-between border border-border-subtle bg-bg-card p-5 hover:border-brand-purple/40 hover:-translate-y-1 hover:shadow-[0_0_20px_rgba(139,92,246,0.15)] transition-all duration-300">
