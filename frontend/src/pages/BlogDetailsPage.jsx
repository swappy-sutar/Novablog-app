  ...
  if (loading) {
    return (
      ...
      >
        ...
        <div className="relative w-full h-[360px] sm:h-[420px] bg-slate-100 dark:bg-white/[0.02] border-b border-gray-200/30 dark:border-border-subtle/20 flex flex-col justify-end overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-6 w-full pb-10 z-20 space-y-4">
            {/* Category tag */}
            <div className="h-5 w-16 bg-slate-300 dark:bg-white/[0.08] border border-slate-400/20 dark:border-white/[0.05] animate-pulse rounded-lg" />
            
            {/* Article Title */}
            <div className="space-y-3">
              <div className="h-10 w-3/4 sm:w-2/3 bg-slate-300 dark:bg-white/[0.08] animate-pulse rounded-md" />
              <div className="h-10 w-1/2 sm:w-1/3 bg-slate-300 dark:bg-white/[0.08] animate-pulse rounded-md" />
            </div>

            {/* Author Meta Details */}
            <div className="flex items-center gap-3 pt-2">
              <div className="w-8 h-8 rounded-full bg-slate-300 dark:bg-white/[0.08] animate-pulse" />
              <div className="h-4 w-24 bg-slate-300/80 dark:bg-white/[0.06] animate-pulse rounded" />
              <div className="w-1.5 h-1.5 rounded-full bg-slate-300/50 dark:bg-white/[0.02] animate-pulse" />
              <div className="h-4 w-16 bg-slate-300/80 dark:bg-white/[0.06] animate-pulse rounded" />
            </div>
          </div>
        </div>

        {/* Share/Actions Toolbar Skeleton */}
        <div className="sticky top-[64px] z-30 w-full h-14 bg-bg-card/75 dark:bg-bg-card/75 backdrop-blur-md border-b border-border-subtle/20 flex items-center justify-center gap-8">
          {Array(4).fill(null).map((_, idx) => (
            <div key={idx} className="w-5 h-5 rounded-full bg-slate-300 dark:bg-white/[0.08] animate-pulse" />
          ))}
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row items-start gap-12 lg:gap-16 pt-12 relative">
          ...
          <div className="hidden lg:block w-64 shrink-0 space-y-6">
            ...
            <div className="space-y-2">
              <div className="h-4 w-5/6 bg-slate-300/70 dark:bg-white/[0.05] animate-pulse rounded" />
              <div className="h-4 w-4/5 bg-slate-300/70 dark:bg-white/[0.05] animate-pulse rounded" />
            </div>
            <div className="h-px w-full bg-slate-200 dark:bg-white/[0.05]" />
            <div className="h-4 w-32 bg-slate-300/80 dark:bg-white/[0.06] animate-pulse rounded" />
          </div>

          {/* Main article lines placeholder */}
          <div className="flex-grow w-full max-w-3xl space-y-8">
            <div className="space-y-3">
              <div className="h-4 w-full bg-slate-300/70 dark:bg-white/[0.05] animate-pulse rounded" />
              <div className="h-4 w-full bg-slate-300/70 dark:bg-white/[0.05] animate-pulse rounded" />
              <div className="h-4 w-11/12 bg-slate-300/70 dark:bg-white/[0.05] animate-pulse rounded" />
              <div className="h-4 w-4/5 bg-slate-300/70 dark:bg-white/[0.05] animate-pulse rounded" />
            </div>
